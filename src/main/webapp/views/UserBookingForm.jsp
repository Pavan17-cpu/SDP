                    ...
                    <form action="/makeBookingForm" modelAttribute="makeBookingForm" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" class="form-control" id="email" name="email"
                                aria-describedby="emailHelp" value="${User_email}" readonly />
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" class="form-control" id="contactno" name="contactno"
                                placeholder="Phone Number" value="${User_phone}" readonly>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="hotel_book" name="hotel_id" class="form-control" required>
                                ...
                                <c:forEach var="hotelData" items="${hotel_for_booking}">
                                    <option id="${hotelData.id}" value="${hotelData.id}">${hotelData.hotelName} -
                                        Per Hour Rs.${hotelData.price}</option>
                                </c:forEach>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="catering_book" name="catering_id" class="form-control" required>
                                ...
                                <c:forEach var="caterData" items="${catering_for_booking}">
                                    <option id="${caterData.id}" value="${caterData.id}">${caterData.catername} -
                                        Per Person Rs.${caterData.cater_price}</option>
                                </c:forEach>
                            </select>
                        </div>
                            ...
                            <select id="photo_book" name="photographer_name_desc" class="form-control" required>
                                ...
                                <c:forEach var="vendorData" items="${vendor_for_booking}">
                                    <c:if test="${vendorData.vendor_desc == 'Photographer'}">
                                        <option id="${vendorData.id}"
                                            value="${vendorData.vendorname}(${vendorData.id})">${vendorData.vendorname}
                                            - ${vendorData.vendor_location} - Rs.${vendorData.vendor_price}</option>
                                    </c:if>
                                </c:forEach>
                        ...
                        <div class="form-group form-check">
                            ...
                            <select id="makeup_book" name="makeupartist_name_desc" class="form-control" required>
                                <option value="">Choose the MakeUp Artist</option>
                                <c:forEach var="vendorData" items="${vendor_for_booking}">
                        ...
                        <div class="form-group form-check">
                            ...
                            <br>
                            <select id="decor_book" name="decorator_name_desc" class="form-control" required>
                                ...
                                <option id="decor_none" value="none">None</option>
                            </select>
                        </div>

                        <!-- Total Amount -->
                     <!-- Total Amount -->
<div class="form-group col-lg-5 mx-auto mb-3 text-center">
    <label for="amt">Total Amount</label>
    <input type="text" class="form-control text-center" id="amt" name="amount" value="0" placeholder="Amount" readonly>
</div>

<!-- Calculate Amount Button -->
<div class="form-group col-lg-5 mx-auto mb-3 text-center">
    <span class="btn btn-dark btn-block py-2 font-weight-bold" name="Calculate"
        onclick="calculateAndToggleInvoice();">Calculate Amount</span>
</div>
						<div class="form-group col-lg-5 mx-auto mb-0 text-center">
							<button type="submit" class="btn btn-dark btn-block py-2 font-weight-bold" name="adduser" onclick="return calc();">
								Book your Event
							</button>
						</div>
...
<script>
    ...
    // Update the total amount based on selected options
    function updateTotalAmount() {
        var totalAmount = (hotelPrice * noOfHours) + (cateringPrice * noOfGuest) + vendorPrice;
        document.getElementById("amt").value = totalAmount;
    }

    // Handle changes in the Hotel dropdown
    $('#hotel_book').change(function () {
        var selectedOption = $('#hotel_book option:selected');
        if (selectedOption.val() !== "") {
            hotelPrice = parseInt(selectedOption.text().match(/\d+/)[0]);
        } else {
            hotelPrice = 0;
        }
        updateTotalAmount();
    ...
    // Handle changes in the Catering dropdown
    $('#catering_book').change(function () {
        var selectedOption = $('#catering_book option:selected');
        if (selectedOption.val() !== "") {
            cateringPrice = parseInt(selectedOption.text().match(/\d+/)[0]);
        } else {
            cateringPrice = 0;
        }
        updateTotalAmount();
    ...
    // Handle changes in the Event Timing dropdown
    $('#event_maxhour').change(function () {
        noOfHours = parseInt($(this).val());
        updateTotalAmount();
    });

    // Handle changes in the Total Number of Guests input
    $('#no_of_guest').on('input', function () {
        noOfGuest = parseInt($(this).val());
        updateTotalAmount();
    });

    // Handle changes in the Photographer dropdown
    $('#photo_book').change(function () {
        var selectedOption = $('#photo_book option:selected');
        if (selectedOption.val() === "none") {
            photoPrice = 0;
        } else {
            photoPrice = parseInt(selectedOption.text().match(/\d+/)[0]);
        }
        updateTotalAmount();
    });

    // Handle changes in the Disc Jockey dropdown
    $('#dj_book').change(function () {
        var selectedOption = $('#dj_book option:selected');
        if (selectedOption.val() === "none") {
            djPrice = 0;
        } else {
            djPrice = parseInt(selectedOption.text().match(/\d+/)[0]);
        }
        updateTotalAmount();
    });

    // Handle changes in the Makeup Artist dropdown
    $('#makeup_book').change(function () {
        var selectedOption = $('#makeup_book option:selected');
        if (selectedOption.val() === "none") {
            makeupPrice = 0;
        } else {
            makeupPrice = parseInt(selectedOption.text().match(/\d+/)[0]);
        }
        updateTotalAmount();
    });

    // Handle changes in the Decorator dropdown
    $('#decor_book').change(function () {
        var selectedOption = $('#decor_book option:selected');
        if (selectedOption.val() === "none") {
            decoratorPrice = 0;
        } else {
            decoratorPrice = parseInt(selectedOption.text().match(/\d+/)[0]);
        }
        updateTotalAmount();
    });

    // Calculate the total amount and show/hide the invoice
    function calculateAndToggleInvoice() {
        // Calculate the total amount here based on your logic
        var totalAmount = hotelPrice * noOfHours + cateringPrice * noOfGuest + photoPrice + djPrice + makeupPrice + decoratorPrice;

        // Update the total amount input field
        $('#amt').val(totalAmount);

        // Toggle the visibility of the invoice section
        var invoiceSection = document.getElementById('invoiceSection');
        if (invoiceSection.style.display === 'none') {
            invoiceSection.style.display = 'block'; // Show the invoice
        } else {
            invoiceSection.style.display = 'none'; // Hide the invoice
        }
    }
